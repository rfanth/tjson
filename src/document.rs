...
pub use crate::tree::{BareForm, KeyForm, MultilineFlavor, StringForm};
...
impl FromStr for Document {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let root = crate::parse::Parser::<Node>::parse_document(s, crate::options::ParseOptions::default()).map_err(Error::Parse)?;
        Ok(Self { root })
    }
}
...
mod tests {
    ...
    fn records_string_and_key_forms() {
        ...
        let root = d.root();
        assert_eq!(
            entry(root, "bare").value().string_form(),
            Some(StringForm::Bare(BareForm::Plain))
        );
    ...
    #[test]
    fn a_recorded_fence_with_no_linefeed_is_not_re_emitted_as_a_fence() {
        // The parser accepts a fence around one unbroken line -- the spec's
        // linefeed rule is a SHOULD -- and records that it was a fence. The
        // generator's rule is the harder one: it emits a fence only for a value
        // that really holds a data EOL, and a recorded form does not buy an
        // exemption. Recorded forms lose to what the value can actually be
        // written as elsewhere too (a recorded `Bare` on a string that is no
        // longer bare-eligible), so the shape of this test is the usual one:
        // the form is dropped, and the data survives the drop.
        for input in ["  doc: ```\nhello\n   ```", "  doc: ``\n| hello\n   ``", "  doc: `\n    hello\n   `"] {
            let d = doc(input);
            let rendered = render_default(&d);
            assert!(
                !rendered.contains('`'),
                "no fence may come back out of {input:?}: {rendered}"
            );
            assert_eq!(doc(&rendered).to_value(), d.to_value(), "data survives: {rendered}");
        }
    }

    #[test]
    fn round_trip_preserves_which_opener_a_bare_string_was_written_with() {
        // Which opener a person wrote is a choice they made, not a fact about
        // the data, so it is recorded and honored like any other form. The
        // marked one is the interesting direction: the default style would
        // write a space, and honoring the record has to beat the style.
        let d = doc("  plain: one\n  marked:_two");
        assert_eq!(
            entry(d.root(), "plain").value().string_form(),
            Some(StringForm::Bare(BareForm::Plain))
        );
        assert_eq!(
            entry(d.root(), "marked").value().string_form(),
            Some(StringForm::Bare(BareForm::Marked))
        );

        let rendered = render_default(&d);
        assert!(rendered.contains("plain: one"), "plain stays plain: {rendered}");
        assert!(rendered.contains("marked:_two"), "marked stays marked: {rendered}");
        assert_eq!(doc(&rendered).to_value(), d.to_value(), "and the data is unmoved");

        // With honoring off, the global style decides and both come out plain.
        let normalized =
            d.to_tjson_with(crate::RenderOptions::default().honor_string_forms(false));
        assert!(!normalized.contains('_'), "no marker survives normalizing: {normalized}");
    }
